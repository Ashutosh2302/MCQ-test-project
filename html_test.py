...
while(True):
    os.system('cls')
    print("1. Create HTML test table")
    ...
    if x==1:
        s="CREATE table htmlTest(question_id int PRIMARY KEY,question varchar(200),option1 varchar(100),option2 varchar(100),option3 varchar(100),option4 varchar(100),correct_ans int)"
        try:
         cur.execute(s)
    ...
    if x==2:
        s="INSERT into htmlTest VALUES(%s,%s,%s,%s,%s,%s,%s)"
    ...
    if x==4:
        s="DELETE FROM htmlTest WHERE question_id=%s"
        a=input("Enter Question ID: ")
        try:
            cur.execute(s,(a,))
        except mysql.connector.errors.ProgrammingError:
